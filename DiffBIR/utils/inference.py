import os
from typing import overload, Generator, Dict, Tuple
...
from utils.helpers import (
    Pipeline,
    BSRNetPipeline, SwinIRPipeline, SCUNetPipeline, Task1Pipeline,
    bicubic_resize
)
from utils.cond_fn import MSEGuidance, WeightedMSEGuidance

from Task1utils import median_filter_4d, cosine_similarity, generate_file_list
...
class Task1InferenceLoop(InferenceLoop):
    @count_vram_usage
    def init_stage1_model(self) -> None:
    ...
    def run(self) -> None:
        ...
        for idx, (hq, lq, prompt) in enumerate(loader):
            lq = lq.squeeze(0)  
            lq = lq.numpy()
            ...
            hq_sample = self.pipeline.run(
                hq[None], self.args.steps, 1.0, self.args.tiled,
                self.args.tile_size, self.args.tile_stride,
                self.args.pos_prompt, self.args.neg_prompt, self.args.cfg_scale,
                self.args.better_start
            )[0]
            ...
            # Median 
            smoothed_lq_sample = median_filter_4d(lq_sample)
            smoothed_hq_sample = median_filter_4d(hq_sample)

            print(cosine_similarity(lq_sample, smoothed_lq_sample), cosine_similarity(hq_sample, smoothed_hq_sample), cosine_similarity(lq_sample, hq_sample))
            # self.save(lq, idx)


    def save(self, sample: np.ndarray, index: int) -> None:
        sample = (np.clip(sample, 0, 1) * 255).astype(np.uint8)  # Ensure values are in [0, 255]
        fname = self.loop_ctx[index]
        file_name = f"{fname}_lq.png"
        save_path = os.path.join(self.args.output, file_name)
        Image.fromarray(sample).save(save_path)
        # print(f"Saved result to {save_path}")
    

    def random_selector(self, directory_path, num_images=None, img_exts = [".png", ".jpg", ".jpeg"]):
        all_files = os.listdir(directory_path)
    
        image_files = [f for f in all_files if f.endswith(img_exts)]
        
        if num_images is None:
            num_images = len(image_files)
        
        if len(image_files) < num_images:
            raise ValueError(f"Not enough images in the directory. Found {len(image_files)} images.")
        
        selected_images = random.sample(image_files, num_images)

    
        selected_image_paths = [os.path.join(directory_path, image) for image in selected_images]
    
        return selected_image_paths


    def lq_loader(self) -> Tuple[torch.utils.data.Dataset, Generator[np.ndarray, None, None]]:
        config = 'configs/train/train_stage1.yaml'
        ...
        if os.path.isdir(self.args.input):
            ...
            file_paths = self.random_selector(self.args.input, num_images=83, img_exts=img_exts)
        else:
            assert os.path.splitext(self.args.input)[-1] in img_exts
            file_paths = [self.args.input]

        
        generate_file_list(file_paths, 'file_list_train.txt')

        
        cfg.dataset.train.params.file_list =  'file_list_train.txt'
        ...
        synthesized_loader = DataLoader(dataset=dataset)

        index = 0
        for file_path in file_paths:
            file_name = os.path.basename(file_path)
            self.loop_ctx[index] = file_name
            index += 1

        return synthesized_loader
    ...
    def init_pipeline(self) -> None:
        self.pipeline = Task1Pipeline(self.swinir, self.cldm, self.diffusion, self.cond_fn, self.args.device)
