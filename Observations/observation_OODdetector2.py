...
def main(args):
   ...
   # Initialize ResNet
    ...
    with open(result_path, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['real_num', 'real_denum','real_res' ,'syn_num', 'syn_denum', 'syn_res'])
    ...
    for batch_idx, (real, syn, gt) in enumerate(tqdm(test_loader)):
        ...
        with torch.no_grad():
            ...
            real_clean, real_clean_ft = run_stage1(swin_model=swinir, lq=real, device=device)
            syn_clean, syn_clean_ft = run_stage1(swin_model=swinir, lq=syn, device=device)
            ...
            real_final_prob = real_sim / (real_output + epsilon)
            syn_final_prob = syn_sim / (syn_output + epsilon)

            with open(result_path, mode='a', newline='') as file:
                writer = csv.writer(file)
                for i in range(len(real)):
                    writer.writerow([real_sim.item()
                                    ,real_output.item()
                                    ,real_final_prob.item()
                                    ,syn_sim.item()
                                    ,syn_output.item()
                                    ,syn_final_prob.item()])
